...
export class Router {
    ...
    _callbackWrapper(callback, query, qs, data, config) {
        return new Promise((resolve) => {
            const ctx = new Context(resolve, query, qs, data, config);
            callback(ctx);
        });
    }
    get(url, callback) {
    ...
    emit(method, url, options, data, config) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            Logger.json({ method, url, data, config, query }, 'offline request information', options.logRequestInfo);
            const eventType = `${RequestMethod[method]}:${pathname}`;
            const callback = this._events.get(eventType);
            if (callback) {
                const response = yield this._callbackWrapper(callback, query, queryString || '', data, config);
